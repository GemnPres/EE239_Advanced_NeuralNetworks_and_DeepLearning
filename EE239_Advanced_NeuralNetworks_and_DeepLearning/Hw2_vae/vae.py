...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.hidden_dim = 400
        self.encoder = nn.Sequential(nn.Linear(self.input_size, self.hidden_dim),
                                     nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x_hat = torch.unflatten(z, 1, img_shape[1::])
        ############################################################################################
        #                                      END OF YOUR CODE                                    #
        ############################################################################################
...
def reparametrize(mu, logvar):
    ...
    std = torch.sqrt(var)   # returns sigma, which is std
    z = torch.multiply(std,eps) + mu
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # BCE loss, input and target are not symmetrical, do in order of x_hat, x
    # select reduction sum since the default is mean
    E_z = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum')
    var = torch.exp(logvar)
    D_kl = -0.5*(torch.sum(1+logvar-torch.pow(mu,2)-var))
    loss = E_z+D_kl
    loss /= mu.shape[0]
    ################################################################################################
    #                            END OF YOUR CODE                                                  #
