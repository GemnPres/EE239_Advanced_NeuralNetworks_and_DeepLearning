...
def discriminator_loss(logits_real, logits_fake):
    ...
    label_one = torch.ones(logits_real.shape[0]).type(dtype)
    label_zero = torch.zeros(logits_fake.shape[0]).type(dtype)
    loss = bce_loss(logits_real, label_one) + bce_loss(logits_fake, label_zero)
...
def generator_loss(logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # Generator wants to fool Discriminator, so the loss is calculated w.r.t label 1
    # More 1 from D, less loss for G
    # device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    label_one = torch.ones(logits_fake.shape[0]).type(dtype)
    loss = bce_loss(logits_fake, label_one)
...
def build_dc_classifier(batch_size):
    ...
    d_model = nn.Sequential(nn.Conv2d(1, 32, 5, 1),
                          nn.LeakyReLU(negative_slope=0.01),
                          nn.MaxPool2d(2,2),
                          nn.Conv2d(32, 64, 5, 1),
                          nn.LeakyReLU(negative_slope=0.01),
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    #         layers.append(nn.Unflatten(1, [128,7,7]))              
    #         layers.append(nn.ConvTranspose2d(128, 64, 4, 2, 1))
    #         layers.append(nn.ReLU())
    #         layers.append(nn.BatchNorm2d(64))
    #         layers.append(nn.ConvTranspose2d(64, 1, 4, 2, 1))
    ...
    g_model = nn.Sequential(nn.Linear(noise_dim, 1024),
                        nn.ReLU(),
                        nn.BatchNorm1d(1024),
